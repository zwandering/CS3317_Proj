...
class CDCL_SOLVER:
    ...
    def run(self):
        self.solver = CDCL_VSIDS(self.sentence, self.num_vars)
        print("run with vsids")
        print("Reward:", self.reward)
        print("UCB1:", self.UCB1)
        print("MOSS:", self.MOSS)
        reward = self.solver.run()
        self.sentence = self.solver.sentence
        if len(self.solver.assignment) >= self.num_vars:
            return self.solver.assignment    
        ...
        self.solver = CDCL_LRB(self.sentence, self.num_vars)
        print("run with lrb")
        print("Reward:", self.reward)
        print("UCB1:", self.UCB1)
        print("MOSS:", self.MOSS)
        ...
        self.solver = CDCL_CHB(self.sentence, self.num_vars)
        print("run with chb")
        print("Reward:", self.reward)
        print("UCB1:", self.UCB1)
        print("MOSS:", self.MOSS)
        ...
        while len(self.solver.assignment) < self.num_vars:
    ...
    def run_without_UCB(self, id):
        ...
        while len(self.solver.assignment) < self.num_vars:
            ...
            self.solver.run()
            print(len(self.solver.assignment), len(self.sentence))
            if len(self.solver.assignment) >= self.num_vars:
                return self.solver.assignment
            self.sentence = self.solver.sentence
            if id == 0:
...
class CDCL_VSIDS:
    ...
    def analyze_conflict(self, conflict_ante):  # NOTE: `sentence` is for resolution
        ...
        return backtrack_level, list(learned_clause)
